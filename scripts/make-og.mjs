...
// Rasterizes a brand SVG with sharp, then composites the official DeTars mark
// (public/brand/detars-mark.png, produced by scripts/make-brand-assets.mjs).
// Run locally: `node scripts/make-og.mjs`
...
const cards = [
  { file: "og.png", eyebrow: "AN AI THAT ACTUALLY DOES THE WORK", a: "Toss DeTars one line.", b: "It runs the rest.", sub: "For the personal work that takes more than one chat." },
...
// Official mark, rounded to match the site header's 32px brand chip.
const MARK = 46;
const markRounded = await sharp("public/brand/detars-mark.png")
  .resize(MARK, MARK)
  .composite([
    {
      input: Buffer.from(
        `<svg width="${MARK}" height="${MARK}"><rect width="${MARK}" height="${MARK}" rx="13" fill="#fff"/></svg>`
      ),
      blend: "dest-in"
    }
  ])
...
for (const c of cards) {
  await sharp(Buffer.from(svg(c)))
    .composite([{ input: markRounded, left: 80, top: 70 }])
    .png()
    .toFile(`public/og/${c.file}`);
